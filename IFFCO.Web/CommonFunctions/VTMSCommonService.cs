    ...
    public class VTMSCommonService
    {
        ...
        public string GetVTProfileImage(string EmpId, int unit)
        {
            try
            {
                ...
                return this.ConvertBlobToString(_context.VtmsEnrollDoc.Where(x => x.VtCode == EmpId).Where(x => x.UnitCode == unit).Select(x => x.VtPhoto).FirstOrDefault());
            }
            catch (NullReferenceException)
            {
        ...
        public string GetVTIdProofStr(string EmpId, int unit)
        {
            try
            {
                //return _context.VtmsEnrollDoc.Where(x => x.VtCode == EmpId).Where(x => x.UnitCode == unit).FirstOrDefault().VtIdUpload;
                return this.ConvertBlobToString(_context.VtmsEnrollDoc.Where(x => x.VtCode == EmpId).Where(x => x.UnitCode == unit).Select(x => x.VtIdUpload).FirstOrDefault());
        ...
        public byte[] GetVTIdProof(string EmpId, int unit)
        {
            ...
            {
                //return _context.VtmsEnrollDoc.Where(x => x.VtCode == EmpId).Where(x => x.UnitCode == unit).FirstOrDefault().VtIdUpload;
                return _context.VtmsEnrollDoc.Where(x => x.VtCode == EmpId).Where(x => x.UnitCode == unit).Select(x => x.VtIdUpload).FirstOrDefault();
                //return this.ConvertPdfBlobToString(_context.VtmsEnrollDoc.Where(x => x.Vtcode == EmpId).Where(x => x.UnitCode == unit).FirstOrDefault().VtIdUpload);
            }
            catch (NullReferenceException)
            {
        ...
        public List<VCompleteVTInfo> VtCompleteDTl()
        {
            ...
            sqlquery += " RECOMM_PNO,RECOMM_ID,RECOMM_NAME,OTHERS_RECOMM_NAME, CERT_FLAG FROM V_COMPLETE_VT_INFO ";
            ...
            DTL_VALUE = (from DataRow dr in dtDTL_VALUE.Rows
                         select new VCompleteVTInfo()
                         {
                             ...
                             Status = Convert.ToString(dr["STATUS"]),
                             StatusDesc = Convert.ToString(dr["STATUS_DESC"]),
        ...
        public List<VCompleteVTInfo> VtCompleteDTl_Find(string vtcode = null)
        {

            string sqlquery = "SELECT VT_CODE, UNIT_CODE, NAME, FATHER_NAME, CONTACT_NO, ADDRESS, DISTRICT_NAME, STATE_NAME, PINCODE, ENROLLED_ON, BRANCH_DESC , COURSE_DESC,   ";
            sqlquery += " DOC_NAME, DOC_REGISTRATION_NO, RECOMMENDATION_TYPE, VT_START_DATE, VT_END_DATE, STATUS, STATUS_DESC, YEAR, SEMESTER, BRANCH_NAME, INSTITUTE_NAME, UNIVERSITY_NAME,   ";
            sqlquery += " RECOMM_PNO,RECOMM_ID,RECOMM_NAME,OTHERS_RECOMM_NAME, CERT_FLAG" +
                ", (SELECT Z.VT_PHOTO FROM VTMS_ENROLL_DOC Z WHERE Z.VT_CODE = VT_CODE AND ROWNUM = 1) VT_PHOTO " +
                "FROM V_COMPLETE_VT_INFO " +
            ...
            byte[] idproof = dtDTL_VALUE != null && dtDTL_VALUE.Rows.Count > 0 && dtDTL_VALUE.Rows[0]["VT_PHOTO"] != DBNull.Value && dtDTL_VALUE.Rows[0]["VT_PHOTO"] != null ? ((byte[])dtDTL_VALUE.Rows[0]["VT_PHOTO"]) : null;
